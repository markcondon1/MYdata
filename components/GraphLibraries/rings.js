...
function Lines({data, maxLength, pressHandler}) {
  ...
  let height = 300 //((width * .1) * (data.length + 1)) + (data.length * .075);
  ...
  for(let i = 0; i < data.length; i++) {
    ...
    toReturn.push(<Circle key={i} cx={width / 2} cy={height / 2} strokeWidth={temp * 2} r={i + 5} stroke={colorArray[data[i].ButtonID]} opacity=".5" onPress={() => pressHandler(data[i])}></Circle>)
    curHeight += width * .1;
  }
